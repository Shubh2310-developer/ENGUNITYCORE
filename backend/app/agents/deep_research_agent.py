...
Answer:
    The DeepResearchAgent class in this file — a multi-step iterative
    research pipeline that decomposes queries into sub-questions, fans out
    parallel async searches across RAG, web, and a knowledge graph, evaluates
    and scores every source, identifies coverage gaps with an LLM, refines
    sub-queries, and synthesizes a structured report. It also exposes a fully
    streaming variant via Python async generators. The core complexity comes
    from multi-level asyncio concurrency, an iterative feedback loop with
    convergence criteria, four distinct LLM call patterns with fallback
    handling, and cross-backend result normalisation.
"""
    ...
    async def research(
        ...
        try:
            ...
            for iteration in range(iterations_to_run):
                ...
                sub_queries = [
                    SubQuery(id=f"refine_{iteration}_{i}", question=q, query_type="exploratory")
    ...
    async def stream_research(
        ...
        for iteration in range(total_max_iterations):
            ...
            yield ResearchStreamEvent(
                event_type="status",
                data={
                    "message": f"Found {len(iteration_sources)} raw sources",
            ...
            for source in iteration_sources[:3]:
                content_preview = source.get("content", "") or source.get("answer", "")
                yield ResearchStreamEvent(
...
Return ONLY the JSON array, no other text."""
    ...
    async def _parallel_search(
        ...
        for sq in sub_queries:
            ...
            if include_web:
                search_tasks.append(self.web_search.search(sq.question))
            if include_graph:
                search_tasks.append(self._graph_search_wrapper(sq.question, user_id))
...
Return ONLY a single float number between 0.0 and 1.0."""
    ...
    def _assess_quality(self, content: str) -> float:
        ...
        if "```" in content or any(
            kw in content.lower()
            for kw in ["average", "percent", "figure", "table", "conclusion"]
        ):
            score += 0.2
