...
FROM dataset`;
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {chartTypes.map((type) => (
                      <button
                        ...
                        className={`flex items-center gap-2 p-3 border rounded-md transition-colors ${chartForm.type === type.id
              ...
              {queryHistory.slice(0, 10).map((query) => (
                <div key={query.id} className="p-3 hover:bg-slate-50 transition-colors cursor-pointer"
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${query.type === 'SQL' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                          }`}>
                          {query.type}
                        </span>
                        <span className="text-xs text-slate-500">
                    ...
                    <div className="flex items-center gap-1 ml-3">
                      <button
                        ...
                        className={`p-1 transition-colors ${query.favorite ? 'text-yellow-500' : 'text-slate-400 hover:text-yellow-500'
                          }`}
                      >
          ...
          <div className="space-y-3">
            ...
            {sqlQuery.trim() && (
              <div className="flex items-center gap-4">
                <div className={`flex items-center gap-2 text-xs px-3 py-1 rounded ${isQueryValid
              ...
              {anomalyAlerts.filter(alert => !alert.dismissed).map((alert) => (
                <div key={alert.id} className="p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${alert.severity === 'high' ? 'bg-red-100 text-red-800' :
          ...
          <div className="flex items-center justify-between">
            <div className="flex space-x-1">
              {tourSteps.map((_, index) => (
                <div
                  key={index}
                  className={`w-2 h-2 rounded-full ${index === tourStep ? 'bg-blue-600' : 'bg-slate-300'
                    }`}
                />
              ))}
            </div>
  ...
  return (
    <div className={`${styles['analytics-theme']} min-h-screen transition-colors ${theme.isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      {/* Top Toolbar */}
      <header className={`sticky top-0 z-30 backdrop-blur-lg border-b transition-colors ${theme.isDark ? 'bg-slate-800/90 border-slate-700' : 'bg-white/90 border-slate-200'
        }`}>
        <div className="max-w-screen-2xl mx-auto px-6 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 text-sm">
                <select
                  ...
                  className={`appearance-none bg-transparent font-semibold p-1 -ml-1 rounded-md transition-colors ${theme.isDark
            ...
            <div className="flex items-center gap-3">
              ...
              <div className={`px-3 py-2 rounded-md text-sm font-medium ${theme.isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-100 text-slate-700'
                }`}>
                GPT-OSS-120B
              </div>
              ...
              <button
                onClick={toggleTheme}
                className={`p-2 rounded-md transition-colors ${theme.isDark ? 'text-slate-300 hover:bg-slate-700' : 'text-slate-600 hover:bg-slate-100'
                  }`}
                title="Toggle theme"
              >
              ...
              <button
                ...
                className={`${styles.btnPrimary} disabled:opacity-50 disabled:cursor-not-allowed`}
              ...
              {/* Share Button */}
              <button
                className={`px-3 py-2 text-sm font-semibold rounded-md transition-colors ${theme.isDark
              ...
              <button
                ...
                className={`${styles.btnGreen} disabled:opacity-50 disabled:cursor-not-allowed`}
      ...
      <main className="flex h-[calc(100vh-69px)] max-w-screen-2xl mx-auto">
            ...
            <div className="mb-6">
              ...
              {uploadedFiles.length === 0 ? (
                <div className="relative">
                  ...
                  />
                  <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${theme.isDark
                    ? 'border-slate-600 hover:border-blue-500 hover:bg-slate-700/50'
                    : 'border-slate-300 hover:border-blue-500 hover:bg-blue-50'
                    } ${isLoading.upload ? 'cursor-wait' : ''}`}>
                    <div className={`w-12 h-12 mx-auto mb-4 rounded-full flex items-center justify-center ${theme.isDark ? 'bg-slate-700' : 'bg-slate-100'
                      }`}>
                      {isLoading.upload ? (
                    ...
                    <p className={`font-semibold mb-1 ${theme.isDark ? 'text-slate-300' : 'text-slate-700'
                      }`}>
                ...
                <div className="space-y-3">
                  {uploadedFiles?.map((file) => (
                    <div key={file.fileId} className={`rounded-lg p-3 border transition-colors ${currentFileId === file.fileId
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <span className={`font-semibold text-sm flex items-center gap-2 ${theme.isDark ? 'text-slate-200' : 'text-slate-800'
                          }`}>
                      ...
                      <div className={`grid grid-cols-2 gap-2 text-sm pt-2 border-t ${theme.isDark ? 'border-slate-600' : 'border-slate-200'
                        }`}>
                        <div>
                          <span className="text-slate-500">Rows:</span>
                          <span className={`ml-2 font-semibold ${theme.isDark ? 'text-slate-300' : 'text-slate-700'
                            }`}>
                  ...
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className={`w-full py-2 px-3 rounded-lg text-sm font-semibold transition-colors ${theme.isDark
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={key} className={`flex items-center justify-between p-2 rounded-lg transition-colors ${theme.isDark ? 'bg-slate-700' : 'bg-white'
                    }`}>
                    <label htmlFor={key} className={`text-sm font-medium cursor-pointer ${theme.isDark ? 'text-slate-300' : 'text-slate-700'
                      }`}>
                      {label}
                    </label>
        ...
        <div className={`flex-1 flex flex-col transition-colors ${theme.isDark ? 'bg-slate-800' : 'bg-white'
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  {isLoading.summary ? (
                    ...
                    <>
                      <div className="grid grid-cols-4 gap-6">
                        <div className={`rounded-xl p-5 border transition-colors ${theme.isDark ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-200'
                          }`}>
                      ...
                      <div className={`rounded-xl p-6 border transition-colors ${theme.isDark ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-200'
                        }`}>
                        <h3 className={`text-lg font-bold mb-4 ${theme.isDark ? 'text-slate-100' : 'text-slate-800'}`}>
                          Dataset Summary
                        </h3>
          ...
          <div className={`border-t p-6 transition-colors ${theme.isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'
            }`}>
      ...
      {Object.values(isLoading).some(loading => loading) && (
        <div className="fixed bottom-4 right-4 z-40">
          <div className={`px-4 py-2 rounded-lg shadow-lg flex items-center gap-3 ${theme.isDark ? 'bg-slate-700 text-slate-200' : 'bg-white text-slate-800'
            }`}>
