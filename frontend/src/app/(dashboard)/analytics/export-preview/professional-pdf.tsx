...
export const generateProfessionalPDF = async (analysisData: AnalysisData, setIsGeneratingPDF: (loading: boolean) => void) => {
  ...
  try {
    ...
    if (analysisData.customCharts) {
      console.log('🎯 Chart IDs to look for:', analysisData.customCharts.map((chart: any, i: number) => `chart-${chart.id || i}`));
    }
    ...
    const BRAND = {
      primary: [59, 130, 246] as [number, number, number],    // Blue-600 (Headers, accents)
      secondary: [51, 65, 85] as [number, number, number],    // Slate-700 (Text)
      light: [248, 250, 252] as [number, number, number],     // Slate-50 (Backgrounds)
      gray: [100, 116, 139] as [number, number, number],      // Slate-500 (Captions)
    ...
    if ((analysisData.customCharts?.length ?? 0) > 0) {
      addSection('Data Visualizations Overview');

      // Add summary
      setStyle('body');
      addText(`This analysis includes ${analysisData.customCharts?.length || 0} custom visualization${(analysisData.customCharts?.length || 0) > 1 ? 's' : ''} that provide insights into data patterns and relationships.`, 'body');
      ...
      for (let i = 0; i < (analysisData.customCharts?.length || 0); i++) {
